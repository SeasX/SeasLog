...
#define SEASLOG_ZEND_HASH_GET_CURRENT_KEY(ht, key, idx)                       \
    zend_hash_get_current_key(ht, key, idx)
#define SEASLOG_ZEND_HASH_INDEX_UPDATE(ht, h, pData, nDataSize, pDest)        \
    zend_hash_index_update_ptr(ht, h, pData)
...
#define SEASLOG_ADD_ASSOC_ZVAL_EX(z, s, l, zn) add_assoc_zval_ex(z, s, l, zn)
#define SEASLOG_ADD_ASSOC_STRING_EX(a, k, l, s)                               \
    add_assoc_string_ex(a, k, l, s, 1)
#define SEASLOG_ADD_NEXT_INDEX_STRING(a, s) add_next_index_string(a, s, 1)
#define SEASLOG_ADD_NEXT_INDEX_STRINGL(a, s, l)                               \
    add_next_index_stringl(a, s, l, 1)
#define SEASLOG_ZEND_HASH_GET_CURRENT_KEY(ht, key, idx)                       \
...
static int _seaslog_log_content(int argc, char *level, char *message,
    int message_len, HashTable *content, char *module, int module_len,
    zend_class_entry *ce TSRMLS_DC);

static int _seaslog_log(int argc, char *level, char *message, int message_len,
    char *module, int module_len, zend_class_entry *ce TSRMLS_DC);

static int _php_log_ex(char *message, int message_len, char *log_file_path,
    int log_file_path_len, zend_class_entry *ce TSRMLS_DC);

static php_stream *seaslog_stream_open_wrapper(char *opt TSRMLS_DC);

static char *getHostName();

static int appender_handle_file(char *message, int message_len, char *level,
    logger_entry_t *logger, zend_class_entry *ce TSRMLS_DC);

static int appender_handle_tcp_udp(char *message, int message_len, char *level,
    logger_entry_t *logger, zend_class_entry *ce TSRMLS_DC);

static char *str_replace(char *ori, char * rep, char * with);

static char *seaslog_format_date(char *format, int format_len,
    time_t ts TSRMLS_DC);

void (*old_error_cb)(int type, const char *error_filename,
    const uint error_lineno, const char *format, va_list args);

void seaslog_error_cb(int type, const char *error_filename,
    const uint error_lineno, const char *format, va_list args);

void (*old_throw_exception_hook)(zval *exception TSRMLS_DC);
void seaslog_throw_exception_hook(zval *exception TSRMLS_DC);

static void process_event(int event_type, int type, char * error_filename,
    uint error_lineno, char * msg TSRMLS_DC);
...
const zend_function_entry seaslog_methods[] = {
    PHP_ME(SEASLOG_RES_NAME, __construct,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_CTOR)
    PHP_ME(SEASLOG_RES_NAME, __destruct,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_DTOR)

    PHP_ME(SEASLOG_RES_NAME, setBasePath,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, getBasePath,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, setLogger,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, getLastLogger,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, setDatetimeFormat,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, getDatetimeFormat,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, analyzerCount,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, analyzerDetail,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, getBuffer,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, flushBuffer,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)

    PHP_ME(SEASLOG_RES_NAME, log,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, debug,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, info,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, notice,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, warning,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, error,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, critical,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, alert,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(SEASLOG_RES_NAME, emergency,
        NULL, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
...
STD_PHP_INI_ENTRY("seaslog.default_basepath", "/var/log/www", PHP_INI_ALL,
    OnUpdateString, default_basepath, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_ENTRY("seaslog.default_logger", "default", PHP_INI_ALL,
    OnUpdateString, default_logger, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_ENTRY("seaslog.default_datetime_format", "Y:m:d H:i:s", PHP_INI_ALL,
    OnUpdateString, default_datetime_format, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_ENTRY("seaslog.logger", "default", PHP_INI_ALL,
    OnUpdateString, logger, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_BOOLEAN("seaslog.disting_type", "0", PHP_INI_ALL,
    OnUpdateBool, disting_type, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_BOOLEAN("seaslog.disting_by_hour", "0", PHP_INI_ALL,
    OnUpdateBool, disting_by_hour, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_BOOLEAN("seaslog.use_buffer", "0", PHP_INI_ALL,
    OnUpdateBool, use_buffer, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_BOOLEAN("seaslog.trace_error", "1", PHP_INI_ALL,
    OnUpdateBool, trace_error, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_BOOLEAN("seaslog.trace_exception", "0", PHP_INI_ALL,
    OnUpdateBool, trace_exception, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_ENTRY("seaslog.buffer_size", "0", PHP_INI_ALL,
    OnUpdateLongGEZero, buffer_size, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_ENTRY("seaslog.level", "0", PHP_INI_ALL,
    OnUpdateLongGEZero, level, zend_seaslog_globals, seaslog_globals)


STD_PHP_INI_ENTRY("seaslog.appender", "1", PHP_INI_ALL,
    OnUpdateLongGEZero, appender, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_ENTRY("seaslog.remote_host", "", PHP_INI_ALL,
    OnUpdateString, remote_host, zend_seaslog_globals, seaslog_globals)

STD_PHP_INI_ENTRY("seaslog.remote_port", "", PHP_INI_ALL,
    OnUpdateLongGEZero, remote_port, zend_seaslog_globals, seaslog_globals)
...
PHP_MINIT_FUNCTION(seaslog)
{
    ...
    REGISTER_STRINGL_CONSTANT("SEASLOG_VERSION",    SEASLOG_VERSION,
        sizeof(SEASLOG_VERSION)   - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_AUTHOR",     SEASLOG_AUTHOR,
        sizeof(SEASLOG_AUTHOR)    - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_ALL",        SEASLOG_ALL,
        sizeof(SEASLOG_ALL)       - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_DEBUG",      SEASLOG_DEBUG,
        sizeof(SEASLOG_DEBUG)     - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_INFO",       SEASLOG_INFO,
        sizeof(SEASLOG_INFO)      - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_NOTICE",     SEASLOG_NOTICE,
        sizeof(SEASLOG_NOTICE)    - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_WARNING",    SEASLOG_WARNING,
        sizeof(SEASLOG_WARNING)   - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_ERROR",      SEASLOG_ERROR,
        sizeof(SEASLOG_ERROR)     - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_CRITICAL",   SEASLOG_CRITICAL,
        sizeof(SEASLOG_CRITICAL)  - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_ALERT",      SEASLOG_ALERT,
        sizeof(SEASLOG_ALERT)     - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_STRINGL_CONSTANT("SEASLOG_EMERGENCY",  SEASLOG_EMERGENCY,
        sizeof(SEASLOG_EMERGENCY) - 1, CONST_PERSISTENT | CONST_CS);

    REGISTER_LONG_CONSTANT("SEASLOG_DETAIL_ORDER_ASC",
        SEASLOG_DETAIL_ORDER_ASC,      CONST_PERSISTENT | CONST_CS);

    REGISTER_LONG_CONSTANT("SEASLOG_DETAIL_ORDER_DESC",
        SEASLOG_DETAIL_ORDER_DESC,     CONST_PERSISTENT | CONST_CS);

    REGISTER_LONG_CONSTANT("SEASLOG_APPENDER_FILE",
        SEASLOG_APPENDER_FILE,         CONST_PERSISTENT | CONST_CS);

    REGISTER_LONG_CONSTANT("SEASLOG_APPENDER_TCP",
        SEASLOG_APPENDER_TCP,          CONST_PERSISTENT | CONST_CS);

    REGISTER_LONG_CONSTANT("SEASLOG_APPENDER_UDP",
        SEASLOG_APPENDER_UDP,          CONST_PERSISTENT | CONST_CS);
...
PHP_MINFO_FUNCTION(seaslog)
{
    ...
    php_info_print_table_row(2,"SeasLog Supports",
    ...
    uint error_lineno, char * msg TSRMLS_DC)
{
    ...
    event_str_len = spprintf(&event_str, 0,
        "%s - type:%d - file:%s - line:%d - msg:%s", event_type_str, type,
        error_filename, error_lineno, msg);

    _seaslog_log(1, SEASLOG_CRITICAL, event_str, event_str_len, NULL, 0,
        seaslog_ce TSRMLS_CC);
}

void seaslog_error_cb(int type, const char *error_filename,
    const uint error_lineno, const char *format, va_list args)
{
    if (type == E_ERROR || type == E_PARSE || type == E_CORE_ERROR ||
        ...
        type == E_RECOVERABLE_ERROR)
    {
        ...
        process_event(SEASLOG_EVENT_ERROR, type, (char *) error_filename,
            error_lineno, msg TSRMLS_CC);
        efree(msg);
    }
...
#if PHP_VERSION_ID >= 70000
    message = zend_read_property(default_ce, exception, "message",
        sizeof("message")-1, 0, &rv);
    file = zend_read_property(default_ce, exception, "file",
        sizeof("file")-1, 0, &rv);
    line = zend_read_property(default_ce, exception, "line",
        sizeof("line")-1, 0, &rv);
    code = zend_read_property(default_ce, exception, "code",
        sizeof("code")-1, 0, &rv);
#else
    message = zend_read_property(default_ce, exception, "message",
        sizeof("message")-1, 0 TSRMLS_CC);
    file = zend_read_property(default_ce, exception, "file",
        sizeof("file")-1, 0 TSRMLS_CC);
    line = zend_read_property(default_ce, exception, "line",
        sizeof("line")-1, 0 TSRMLS_CC);
    code = zend_read_property(default_ce, exception, "code",
        sizeof("code")-1, 0 TSRMLS_CC);
#endif

    process_event(SEASLOG_EVENT_EXCEPTION, Z_LVAL_P(code), Z_STRVAL_P(file),
        Z_LVAL_P(line), Z_STRVAL_P(message) TSRMLS_CC);
...
{
    SEASLOG_G(base_path)                    =
        estrdup(SEASLOG_G(default_basepath));
    SEASLOG_G(current_datetime_format)      =
        estrdup(SEASLOG_G(default_datetime_format));
    SEASLOG_G(current_datetime_format_len)  =
        strlen(SEASLOG_G(current_datetime_format));

    SEASLOG_G(last_logger)                  =
        ecalloc(sizeof(logger_entry_t), 1);

    SEASLOG_G(last_logger)->logger_len      =
        spprintf(&SEASLOG_G(last_logger)->logger, 0, "%s",
            SEASLOG_G(default_logger));

    SEASLOG_G(last_logger)->logger_path_len =
        spprintf(&SEASLOG_G(last_logger)->logger_path, 0, "%s/%s",
            SEASLOG_G(base_path), SEASLOG_G(last_logger)->logger);
    ...
    SEASLOG_G(last_sec)->real_time     =
        seaslog_format_date(SEASLOG_G(current_datetime_format),
            SEASLOG_G(current_datetime_format_len), now TSRMLS_CC);
...
static php_stream *seaslog_stream_open_wrapper(char *opt TSRMLS_DC)
{
    ...
    case SEASLOG_APPENDER_TCP:
        reslen = spprintf(&res, 0, "tcp://%s:%d", SEASLOG_G(remote_host),
            SEASLOG_G(remote_port));
        stream = php_stream_xport_create(res, reslen, REPORT_ERRORS,
            STREAM_XPORT_CLIENT | STREAM_XPORT_CONNECT, 0, 0, NULL, NULL, NULL);
        efree(res);
        break;

    case SEASLOG_APPENDER_UDP:
        reslen = spprintf(&res, 0, "udp://%s:%d", SEASLOG_G(remote_host),
            SEASLOG_G(remote_port));
        stream = php_stream_xport_create(res, reslen, REPORT_ERRORS,
            STREAM_XPORT_CLIENT | STREAM_XPORT_CONNECT, 0, 0, NULL, NULL, NULL);
...
static int seaslog_buffer_set(char *log_info, int log_info_len, char *path,
    int path_len, zend_class_entry *ce TSRMLS_DC)
{
    ...
    if (zend_hash_num_elements(Z_ARRVAL(SEASLOG_G(buffer))) < 1) {
        ...
        SEASLOG_ADD_ASSOC_ZVAL_EX(&SEASLOG_G(buffer), path, path_len,
            &new_array);

    } else {
        zval      *_buffer_data_;

        if ((_buffer_data_ = zend_hash_str_find_ptr(Z_ARRVAL(SEASLOG_G(buffer)),
            path, path_len)) != NULL)
        {
            SEASLOG_ADD_NEXT_INDEX_STRINGL((void *)&_buffer_data_, log_info,
                log_info_len);
    ...
    if (zend_hash_num_elements(Z_ARRVAL_P(SEASLOG_G(buffer))) < 1) {
        ...
        if (zend_hash_find(HASH_OF(SEASLOG_G(buffer)), path, path_len,
            (void**)&_buffer_data_) == SUCCESS)
        {

            convert_to_array_ex(_buffer_data_);
            SEASLOG_ADD_NEXT_INDEX_STRINGL(*_buffer_data_, log_info,
                log_info_len);
...
static char *mk_real_time(TSRMLS_D)
{
    ...
    if (now > SEASLOG_G(last_sec)->sec) {
        ...
        last_sec->sec = now;
        last_sec->real_time = seaslog_format_date(
            SEASLOG_G(current_datetime_format),
            SEASLOG_G(current_datetime_format_len), now TSRMLS_CC);
...
static logger_entry_t *process_logger(char *logger, int logger_len TSRMLS_DC)
{
    ...
    if (zend_hash_index_find(ht, logger_entry_hash, (void*)&logger_entry) == SUCCESS) {
        ...
        logger_entry->logger_path_len = spprintf(&logger_entry->logger_path, 0,
            "%s/%s", SEASLOG_G(base_path), logger_entry->logger);
        ...
        SEASLOG_ZEND_HASH_INDEX_UPDATE(ht, logger_entry_hash, logger_entry,
            sizeof(logger_entry_t), NULL);
...
static long get_type_count(char *log_path, char *level, char *key_word TSRMLS_DC)
{
    ...
    if (SEASLOG_G(disting_type)) {
        spprintf(&path, 0, "%s/%s.%s*", SEASLOG_G(last_logger)->logger_path,
            level, log_path);
    } else {
        spprintf(&path, 0, "%s/%s*",    SEASLOG_G(last_logger)->logger_path,
            log_path);
    }
    ...
    if (key_word) {
#ifdef WINDOWS
        spprintf(&sh, 0, "findstr \"%s\" %s | find /c \"%s\" ", level, path,
            key_word);
#else
        spprintf(&sh, 0, "more %s 2>/dev/null| grep '%s' | grep '%s' -wc", path,
            level, key_word);
#endif
    } else {
#ifdef WINDOWS
...
static int get_detail(char *log_path, char *level, char *key_word, long start,
    long end, long order, zval *return_value TSRMLS_DC)
{
    FILE * fp;
...
#ifdef WINDOWS
        spprintf(&sh, 0, "findstr \"%s\" %s | findstr \"%s\" ", level, path,
            key_word);
#else
        spprintf(&sh, 0,
            "%s 2 > /dev/null | grep '%s' -w | grep '%s' -w | sed -n '%ld,%ld'p",
            command, level, key_word, start, end);
#endif
    } else {
#ifdef WINDOWS
        spprintf(&sh, 0, "findstr \"%s\" %s", level, path);
#else
        spprintf(&sh, 0, "%s 2 > /dev/null | grep '%s' -w | sed -n '%ld,%ld'p",
            command, level, start, end);
#endif
    }
...
static void seaslog_log_by_level_common(INTERNAL_FUNCTION_PARAMETERS,
    char *level)
{
    ...
    if (zend_parse_parameters(argc TSRMLS_CC, "S|zS", &message, &content,
        &logger) == FAILURE)
        return;
    ...
    if (argc > 1) {
        if (_seaslog_log_content(argc, level, ZSTR_VAL(message),
            ZSTR_LEN(message), HASH_OF(content), ZSTR_VAL(logger),
            ZSTR_LEN(logger), seaslog_ce TSRMLS_CC) == FAILURE)
        {
            RETURN_FALSE;
        }
    } else {
        if (_seaslog_log(argc, level, ZSTR_VAL(message), ZSTR_LEN(message), "", 
            0, seaslog_ce TSRMLS_CC) == FAILURE)
        {
            RETURN_FALSE;
        }
    }
    ...
    if (zend_parse_parameters(argc TSRMLS_CC, "s|Zs", &message, &message_len,
        &content, &logger, &logger_len) == FAILURE)
        return;
    ...
    if (argc > 1) {
        if (_seaslog_log_content(argc, level, message, message_len,
            HASH_OF(*content), logger, logger_len, seaslog_ce TSRMLS_CC)
            == FAILURE)
        {
            RETURN_FALSE;
        }
    } else {
        if (_seaslog_log(argc, level, message, message_len, "", 0,
            seaslog_ce TSRMLS_CC) == FAILURE)
        {
            RETURN_FALSE;
        }
    }
...
PHP_METHOD(SEASLOG_RES_NAME, setBasePath)
{
    ...
    if (argc > 0 && (Z_TYPE_P(_base_path) == IS_STRING ||
        Z_STRLEN_P(_base_path) > 0))
    {
...
PHP_METHOD(SEASLOG_RES_NAME, setLogger)
{
    ...
    if (argc > 0 && (Z_TYPE_P(_module) == IS_STRING || Z_STRLEN_P(_module) > 0))
    {
        ...
        SEASLOG_G(last_logger) = process_logger(Z_STRVAL_P(_module),
            Z_STRLEN_P(_module) TSRMLS_CC);
...
{
    SEASLOG_RETURN_STRINGL(SEASLOG_G(last_logger)->logger,
        SEASLOG_G(last_logger)->logger_len);
}
...
PHP_METHOD(SEASLOG_RES_NAME, setDatetimeFormat)
{
    ...
    {
        if (!strcmp(SEASLOG_G(current_datetime_format),
            SEASLOG_G(default_datetime_format)))
        {
            efree(SEASLOG_G(current_datetime_format));
        }
...
PHP_METHOD(SEASLOG_RES_NAME, analyzerCount)
{
    ...
    if (zend_parse_parameters(argc TSRMLS_CC, "|sss", &level, &level_len,
        &log_path, &log_path_len, &key_word, &key_word_len) == FAILURE)
        return;

    if (argc == 0 || (argc == 1 && level && !strcmp(level, SEASLOG_ALL))) {
        long count_debug, count_info, count_notice, count_warn, count_erro,
            count_critical, count_alert, count_emergency;
        array_init(return_value);
        log_path = "";
...
PHP_METHOD(SEASLOG_RES_NAME, analyzerDetail)
{
    ...
    if (zend_parse_parameters(argc TSRMLS_CC, "S|SSlll", &_level, &_log_path,
        &_key_word, &start, &limit, &order) == FAILURE)
    {
        RETURN_FALSE;
    }
    ...
    if (zend_parse_parameters(argc TSRMLS_CC, "s|sslll", &level, &level_len,
        &log_path, &log_path_len, &key_word, &key_word_len, &start, &limit,
        &order) == FAILURE)
    {
        RETURN_FALSE;
    }
    ...
    get_detail(log_path, level, key_word, start, start + limit, order,
...
PHP_METHOD(SEASLOG_RES_NAME, log)
{
    ...
    if (zend_parse_parameters(argc TSRMLS_CC, "SS|zS", &level, &message,
        &content, &logger) == FAILURE)
        return;

    if (argc > 2 && Z_TYPE_P(content) != IS_ARRAY) {
        php_error_docref(NULL TSRMLS_CC, E_WARNING,
            "The three argument is not an array");
        RETURN_FALSE;
    }

    if (argc > 3) {
        if (_seaslog_log_content(argc, ZSTR_VAL(level), ZSTR_VAL(message),
            ZSTR_LEN(message), HASH_OF(content), ZSTR_VAL(logger),
            ZSTR_LEN(logger), seaslog_ce TSRMLS_CC) == FAILURE)
        {
            RETURN_FALSE;
        ...
        if (_seaslog_log_content(argc, ZSTR_VAL(level), ZSTR_VAL(message),
            ZSTR_LEN(message), HASH_OF(content), "", 0, seaslog_ce TSRMLS_CC)
            == FAILURE)
        {
            RETURN_FALSE;
        }
    } else {
        if (_seaslog_log(argc, ZSTR_VAL(level), ZSTR_VAL(message),
            ZSTR_LEN(message), "", 0, seaslog_ce TSRMLS_CC) == FAILURE)
        {
            RETURN_FALSE;
        }
    }
    ...
    if (zend_parse_parameters(argc TSRMLS_CC, "ss|Zs", &level, &level_len,
        &message, &message_len, &content, &logger, &logger_len) == FAILURE)
        return;
    ...
    if (argc > 2) {
        ...
        if (_seaslog_log(argc, level, message, message_len, logger, logger_len,
            seaslog_ce TSRMLS_CC) == FAILURE)
        {
            RETURN_FALSE;
        }
...
{
    seaslog_log_by_level_common(INTERNAL_FUNCTION_PARAM_PASSTHRU,
        SEASLOG_DEBUG);
}
...
char *strreplace(char *src, const char *oldstr, const char *newstr, size_t len)
{
    ...
    {
        tmp = (char*) emalloc(strlen(src) + (strlen(newstr) - strlen(oldstr))
            + 1);
    ...
    ZEND_HASH_FOREACH_KEY_VAL(pats, num_key, str_key, entry) {
        if (UNEXPECTED(!str_key)) {
            ...
            if (strstr(str,ZSTR_VAL(str_key))) {
                tmp = strreplace(tmp, ZSTR_VAL(str_key), ZSTR_VAL(s),
                    strlen(str));
            }
...
static char *php_strtr_array(char *str, int slen, HashTable *hash)
{
    ...
    while (zend_hash_get_current_data_ex(hash, (void **)&entry, &hpos)
        == SUCCESS)
    {
        switch (zend_hash_get_current_key_ex(hash, &string_key, &string_key_len,
            &num_key, 0, &hpos))
        {
        case HASH_KEY_IS_STRING:
            ...
            if (len < 1) {
                ...
                zend_hash_add(&tmp_hash, string_key, string_key_len, entry,
                    sizeof(zval*), NULL);
                if (len > maxlen) {
                    maxlen = len;
                }
        ...
        case HASH_KEY_IS_LONG:
            ...
            len = Z_STRLEN(ctmp);
            zend_hash_add(&tmp_hash, Z_STRVAL(ctmp), len + 1, entry,
                sizeof(zval*), NULL);
            zval_dtor(&ctmp);
    ...
    while (pos < slen) {
        ...
        for (len = maxlen; len >= minlen; len--) {
            ...
            if (zend_hash_find(&tmp_hash, key, len + 1, (void**)&trans)
                == SUCCESS)
            {
                char *tval;
                int tlen;
    ...
    zend_class_entry *ce TSRMLS_DC)
{
    ...
    ret = _seaslog_log(argc, level, result, strlen(result), module, module_len,
        ce TSRMLS_CC);
    ...
    char *module, int module_len, zend_class_entry *ce TSRMLS_DC)
{
    ...
    case SEASLOG_APPENDER_TCP:
    case SEASLOG_APPENDER_UDP:
        return appender_handle_tcp_udp(message, message_len, level, logger,
            ce TSRMLS_CC);
        break;
    case SEASLOG_APPENDER_FILE:
    default:
        return appender_handle_file(message, message_len, level, logger,
            ce TSRMLS_CC);
    }
    ...
    logger_entry_t *logger, zend_class_entry *ce TSRMLS_DC)
{
    ...
    if (SEASLOG_G(disting_type)) {
        log_file_path_len = spprintf(&log_file_path, 0, "%s/%s.%s.log",
            logger->logger_path, level, real_date);
    } else {
        log_file_path_len = spprintf(&log_file_path, 0, "%s/%s.log",
            logger->logger_path,real_date);
    }

    log_len = spprintf(&log_info, 0, "%s | %d | %s | %s | %s \n", level,
        getpid(), current_time, real_time, message);


    if (_php_log_ex(log_info, log_len, log_file_path, log_file_path_len + 1,
    ...
    logger_entry_t *logger, zend_class_entry *ce TSRMLS_DC)
{
    ...
    log_len = spprintf(&log_info, 0, "%s | %s | %s | %d | %s | %s | %s \n",
        SEASLOG_G(host_name),logger->logger,level, getpid(), current_time,
        real_time, message);

    if (_php_log_ex(log_info, log_len, logger->logger, logger->logger_len,
        ce TSRMLS_CC) == FAILURE)
    {
        efree(log_info);
        return FAILURE;
...
static int _ck_log_level(char *level TSRMLS_DC) {
    ...
    if (strcmp(level, SEASLOG_DEBUG)      == 0 && SEASLOG_G(level) <= 1)
        return SUCCESS;
    if (strcmp(level, SEASLOG_INFO)       == 0 && SEASLOG_G(level) <= 2)
        return SUCCESS;
    if (strcmp(level, SEASLOG_NOTICE)     == 0 && SEASLOG_G(level) <= 3)
        return SUCCESS;
    if (strcmp(level, SEASLOG_WARNING)    == 0 && SEASLOG_G(level) <= 4)
        return SUCCESS;
    if (strcmp(level, SEASLOG_ERROR)      == 0 && SEASLOG_G(level) <= 5)
        return SUCCESS;
    if (strcmp(level, SEASLOG_CRITICAL)   == 0 && SEASLOG_G(level) <= 6)
        return SUCCESS;
    if (strcmp(level, SEASLOG_ALERT)      == 0 && SEASLOG_G(level) <= 7)
        return SUCCESS;
    if (strcmp(level, SEASLOG_EMERGENCY)  == 0 && SEASLOG_G(level) <= 8)
        return SUCCESS;
...
static int _mk_log_dir(char *dir TSRMLS_DC)
{
    ...
    if (SEASLOG_G(appender) == SEASLOG_APPENDER_FILE) {
        ...
        if (_ck_dir == FAILURE) {
            ...
            if (php_stream_mkdir(dir, mode,
                (recursive ? PHP_STREAM_MKDIR_RECURSIVE : 0) | REPORT_ERRORS,
                context))
            {
                return FAILURE;
            }
...
{
    if (SEASLOG_G(use_buffer)) {
        seaslog_buffer_set(message, message_len, log_file_path,
            log_file_path_len, ce TSRMLS_CC);
        return SUCCESS;
    } else {
